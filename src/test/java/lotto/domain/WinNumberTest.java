...
class WinNumberTest {
    ...
    void 보너스_숫자가_원래_숫자와_겹치면_예외(){
        assertThatThrownBy(()-> {
            WinNumber winNumber = new WinNumber(new LottoTrialManual(List.of(1,2,3,4,5,6)), LottoBallNumber.get(6));
        }).isInstanceOf(DuplicatedBallNumber.class);
    }
    @Test
    void 보너스_숫자가_원래_숫자와_안겹치면_정상실행(){
        assertThatCode(()-> {
            WinNumber winNumber = new WinNumber(new LottoTrialManual(List.of(1,2,3,4,5,6)), LottoBallNumber.get(7));
