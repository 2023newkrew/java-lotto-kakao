...
public class DefaultLottoResultTest {
    @DisplayName("로또 결과를 지정된 값으로 생성한다")
    @ParameterizedTest
    @ValueSource(ints = {0,1,2,3,4,5,6})
    void 로또_결과를_지정된_값으로_생성한다(int count) {
        Assertions.assertThatCode(()-> new DefaultLottoResult(count, false))
    ...
    void 유효하지_않은_결과에_대해_예외를_발생시킨다(int count) {
        Assertions.assertThatThrownBy(()-> new DefaultLottoResult(count, false))
